$(document).ready(function () {
  $("a").tooltip({
    ...
    'trigger' : 'hover'
        ...
        $('#push_message').val(
        $('#push_message').val().replace('{ { campaign.${name} } }',"{{campaign.${name}}}")
        );
        
        $('#push_message').val(
        $('#push_message').val().replace('{ { ${set_user_to_unsubscribed_url} } }',"{{${set_user_to_unsubscribed_url}}}")
        );

        $('#push_message').val(
        $('#push_message').val().replace('{ { ${date_of_birth} } }',"{{${date_of_birth}}}")
        );

        $('#push_message').val(
        $('#push_message').val().replace('{ { ${email_address} } }',"{{${email_address}}}")
        );

        $('#push_message').val(
        $('#push_message').val().replace('{ { ${first_name} } }',"{{${first_name}}}")
        );

        $('#push_message').val(
        $('#push_message').val().replace('{ { ${gender} } }',"{{${gender}}}")
        );

        $('#push_message').val(
        $('#push_message').val().replace('{ { ${last_name} } }',"{{${last_name}}}")
        );

        $('#push_message').val(
        $('#push_message').val().replace('{ { ${last_used_app_date} } }',"{{${last_used_app_date}}}")
        );
        
        $('#push_message').val(
        $('#push_message').val().replace('{ { ${most_recent_app_version} } }',"{{${most_recent_app_version}}}")
        );
        
        $('#push_message').val(
        $('#push_message').val().replace('{ { ${phone_number} } }',"{{${phone_number}}}")
        );
        
        $('#push_message').val(
        $('#push_message').val().replace('{ { ${time_zone} } }',"{{${time_zone}}}")
        );
        
        $('#push_message').val(
        $('#push_message').val().replace('{ { ${company} } }',"{{${company}}}")
        );

        //push_title
        $('#push_title').val(
        $('#push_title').val().replace('{ { campaign.${name} } }',"{{campaign.${name}}}")
        );
        
        $('#push_title').val(
        $('#push_title').val().replace('{ { ${set_user_to_unsubscribed_url} } }',"{{${set_user_to_unsubscribed_url}}}")
        );

        $('#push_title').val(
        $('#push_title').val().replace('{ { ${date_of_birth} } }',"{{${date_of_birth}}}")
        );

        $('#push_title').val(
        $('#push_title').val().replace('{ { ${email_address} } }',"{{${email_address}}}")
        );

        $('#push_title').val(
        $('#push_title').val().replace('{ { ${first_name} } }',"{{${first_name}}}")
        );

        $('#push_title').val(
        $('#push_title').val().replace('{ { ${gender} } }',"{{${gender}}}")
        );

        $('#push_title').val(
        $('#push_title').val().replace('{ { ${last_name} } }',"{{${last_name}}}")
        );

        $('#push_title').val(
        $('#push_title').val().replace('{ { ${last_used_app_date} } }',"{{${last_used_app_date}}}")
        );
        
        $('#push_title').val(
        $('#push_title').val().replace('{ { ${most_recent_app_version} } }',"{{${most_recent_app_version}}}")
        );
        
        $('#push_title').val(
        $('#push_title').val().replace('{ { ${phone_number} } }',"{{${phone_number}}}")
        );
        
        $('#push_title').val(
        $('#push_title').val().replace('{ { ${time_zone} } }',"{{${time_zone}}}")
        );
        
        $('#push_title').val(
        $('#push_title').val().replace('{ { ${company} } }',"{{${company}}}")
        );
...
function validateEmail(){
    ...
    if(errorResult == -1){
    	androidCampaign["email_subject"] = subject;
		androidCampaign["email_message"] = message;
    	...
    	$("#emailCheck").css('background','#00a651');
    }
...
function validateTarget(){
    ...
    if(errorResult == -1){
		...
		$("#ios_standard_preview_message").html(androidCampaign["push_message"].slice(0,47));
		$("#ios_extended_preview_message").html(androidCampaign["push_message"]);
		...
		if(youtubeimgsrc == url && push_img_url == ''){
			...
			if(androidCampaign["selectedPlatform"] == 'iOS'){
				youtubeimgsrc = baseurl+"upload/apps/"+iOSAppImage;
...
function launchCampaign(){
	...
	$.ajax({
		...
		type: "POST",
        url: baseurl + 'crosschannel/saveCrossChannel',
        data: jsonString,
        ...
        success: function(data){
        	...
        	//setTimeout(function(){
        		...
        		if(androidCampaign["campaignId"] != ''){
        			window.parent.location.href =  baseurl+"appUser/editCrossChannel/"+androidCampaign["campaignId"];
        		}else{
        			window.parent.location.href =  baseurl+"appUser/crossChannel";
        		}
...
$("#send-test-mail").click(function(){
    $("#send-test-message, #block-attributes").toggle();
  });
      	...
      	$.ajax({
              ...
              success: function(data){
                //alert(data);
              	/*if(data == "1"){
              	   $(".campaign-loader").css('display','none');
                   $("#email-send-response").html("Email send successfully.");
              	}else{
                   $("#email-send-response").html("Server Busy. Please try again.");
                }*/
...
function campaigns_more_activities_click()
{
    ...
    if (parseInt(totalrecord) > parseInt(statuscount)) {
        ...
        $.ajax({
            type: 'POST',
            url: baseurl + 'crosschannel/crossChannelListPagination',
...
//        }
	...
	composeDraft["send_push_to_recently_used_device"] = send_push_to_recently_used_device;
 	...
 	$.ajax({
 		...
 		type: "POST",
         url: baseurl + 'crosschannel/saveComposeAsDraft',
         data: jsonString,
         contentType: "application/json; charset=utf-8",
         dataType: "json",
         success: function(data){
...
function saveEmailAsDraft(){
	 ...
	 var baseurl = $("#baseurl").val();
	 emailDraft["selectedPlatform"] = androidCampaign["selectedPlatform"];
	 emailDraft["campaignName"] = androidCampaign["campaignName"];
	 emailDraft["custom_url"] = androidCampaign["custom_url"];
	 emailDraft["redirect_url"] = androidCampaign["redirect_url"];
	 emailDraft["expandedImage"] = androidCampaign["expandedImage"];
	 emailDraft["expanded_img_url"] = androidCampaign["expanded_img_url"];
	 emailDraft["groupId"] = androidCampaign["groupId"];
	 emailDraft["push_icon"] = androidCampaign["push_icon"];
	 emailDraft["push_img_url"] = androidCampaign["push_img_url"];
	 emailDraft["campaignId"] = androidCampaign["campaignId"];
	 emailDraft["campaignPersonaUser"] = androidCampaign["campaignPersonaUser"];
	 emailDraft["campaignList"] = androidCampaign["campaignList"];
	 emailDraft["message_category"] = androidCampaign["message_category"];
         emailDraft["automation"] = androidCampaign['automation'];
	 emailDraft["push_message"] = androidCampaign["push_message"];
	 emailDraft["push_title"] = androidCampaign["push_title"];
	 emailDraft["summery_text"] = androidCampaign["summery_text"];
	 emailDraft["send_push_to_recently_used_device"] = androidCampaign["send_push_to_recently_used_device"];
	    ...
	    if(errorResult == -1){
	    	...
	    	emailDraft["displayName"] = displayName;
	    	emailDraft["fromAddress"] = fromAddress;
	    	emailDraft["replyToAddress"] = replyToAddress;
	    	emailDraft["email_subject"] = subject;
	    	emailDraft["email_message"] = message;

	    	jsonEmailObj.push(emailDraft);
	     	jsonString = JSON.stringify(jsonEmailObj);

	     	$(".campaign-loader").css('display','block');
	     	$.ajax({

	     		type: "POST",
	             url: baseurl + 'crosschannel/saveEmailAsDraft',
	             data: jsonString,
	             contentType: "application/json; charset=utf-8",
	             dataType: "json",
	             success: function(data){
...
function saveDeliveryAsDraft(){
	 ...
	 deliveryDraft["send_push_to_recently_used_device"] = androidCampaign["send_push_to_recently_used_device"];
		 ...
		 deliveryDraft["email_subject"] = androidCampaign["email_subject"];;
		 deliveryDraft["email_message"] = androidCampaign["email_message"];
		    ...
		    if(errorResult == -1){
		    	...
		    	jsonDeliveryObj.push(deliveryDraft);
			 	...
			 	$.ajax({
			 		...
			 		type: "POST",
			         url: baseurl + 'crosschannel/saveDeliveryAsDraft',
			         data: jsonString,
			         contentType: "application/json; charset=utf-8",
			         dataType: "json",
			         success: function(data){
...
function saveTargetAsDraft(){
	...
	targetDraft["push_title"] = androidCampaign["push_title"];;
	targetDraft["push_message"] = androidCampaign["push_message"];
	targetDraft["email_subject"] = androidCampaign["email_subject"];
	targetDraft["email_message"] = androidCampaign["email_message"];
			...
			if($('#send_this_push_to_users_most_recently_used_device').is(":checked")){
		    	...
		    	jsonTargetObj.push(targetDraft);
			 	...
			 	$.ajax({
			 		...
			 		type: "POST",
			         url: baseurl + 'crosschannel/saveTargetAsDraft',
			         data: jsonString,
			         contentType: "application/json; charset=utf-8",
			         dataType: "json",
			         success: function(data){
...
function saveConfirmAsDraft(){
	...
	$.ajax({
		...
		type: "POST",
        url: baseurl + 'crosschannel/saveCrossChannelDraft',
        data: jsonString,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function(data){
...
function delete_campaigns(campaignId){
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data == 1){
                window.location.href = baseurl + 'appUser/crossChannel';
            }else{
...
//        }
	...
	automationCampaign["send_push_to_recently_used_device"] = send_push_to_recently_used_device;
 	...
 	$.ajax({
 		...
 		type: "POST",
         url: baseurl + 'crosschannel/saveAutomation',
         data: jsonString,
         contentType: "application/json; charset=utf-8",
         dataType: "json",
         success: function(data){
